...
open class LayoutBaseViewController: BaseViewController {
    
    /// backgroundView
    open lazy var backgroundView : UIView = { return UIView()}()
    
    /// contentView
    open lazy var contentView : UIView = { return UIView()}()
    
    /// titleView
    open lazy var titleView : UIView = { return UIView()}()
    
    /// [子类根据需要重写] baseViews是否使用自动布局，默认为true，如果返回false，则表示手动布局
    open class func baseViewsUseAutoLayout() -> Bool { return true}
     
    /// pop事件,提供给子类的按钮调用
    @objc open func popEvent() { popViewController()}
    
    /// [子类根据需要重写] 构建基础BaseViews
    open func buildBaseViews() {
